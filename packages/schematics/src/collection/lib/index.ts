import {apply, branchAndMerge, chain, mergeWith, noop, Rule, template, Tree, url, Source} from '@angular-devkit/schematics';
...
function addRouterConfiguration(
  ...
  return (host: Tree) => {
    ...
    insert(host, modulePath, [
      insertImport(
          moduleSourceFile, modulePath, 'RouterModule, Route',
          '@angular/router'),
      ...addImportToModule(moduleSourceFile, modulePath, `RouterModule`),
      ...addGlobal(
          moduleSourceFile, modulePath,
...
function addLoadChildren(schema: NormalizedSchema): Rule {
  return (host: Tree) => {
    ...
    const sourceFile = ts.createSourceFile(
        schema.parentModule, moduleSource, ts.ScriptTarget.Latest, true);

    const loadChildren = `@${json.project.npmScope}/${
        toFileName(schema.fullName)}#${toClassName(schema.name)}Module`;
    insert(
        host, schema.parentModule,
        [...addRoute(
            schema.parentModule, sourceFile,
            `{path: '${toFileName(schema.name)}', loadChildren: '${
                loadChildren}'}`)]);
...
function addChildren(schema: NormalizedSchema): Rule {
  return (host: Tree) => {
    ...
    const constName = `${toPropertyName(schema.name)}Routes`;
    const importPath =
        `@${json.project.npmScope}/${toFileName(schema.fullName)}`;
...
function updateTsLint(schema: NormalizedSchema): Rule {
  return (host: Tree) => {
    ...
    if (tsLint['rules'] && tsLint['rules']['nx-enforce-module-boundaries'] &&
        tsLint['rules']['nx-enforce-module-boundaries'][1] &&
        tsLint['rules']['nx-enforce-module-boundaries'][1]['lazyLoad']) {
      tsLint['rules']['nx-enforce-module-boundaries'][1]['lazyLoad'].push(
          toFileName(schema.fullName));
